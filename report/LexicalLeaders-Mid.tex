...
\subsection{Recent Multimodal Foundation-Model Context}
Recent multimodal systems provide useful context for why we keep a parameter-efficient design at mid stage. BLIP-2 \cite{li2023blip2} and Flamingo \cite{alayrac2022flamingo} show that frozen vision encoders with lightweight adapters can be highly competitive. Instruction-tuned vision-language assistants such as LLaVA \cite{liu2023llava} further indicate that alignment quality often depends more on adaptation strategy and data quality than on full end-to-end retraining.

This informs our project choices in two ways. First, we retain frozen CLIP features for compute efficiency and reproducibility in a classroom setting. Second, for planned Phase 2 we prioritize targeted adaptation (LoRA/PEFT + OCR realism + safety policy) over brute-force scaling.

\subsection{Task-Specific Moderation Considerations}
Unlike generic image-text understanding, hateful meme detection has asymmetric error costs: false negatives may allow harmful content to pass, while false positives may suppress benign speech. Because of this asymmetry, we treat moderation as a \emph{risk-sensitive decision process} rather than pure classification. This is why our plan combines predictive metrics (AUROC/F1) with policy-aware operational metrics (escalation rate, reviewer agreement, and auditability).
...
\subsection{Extended Evaluation Protocol (Final Phase Plan)}
Beyond raw point estimates, final-phase evaluation will include protocol-level checks so that reported gains are robust and operationally meaningful.

\textbf{Threshold calibration options.}
\begin{itemize}[leftmargin=*,nosep]
  \item Fixed threshold (0.5) for reproducible baseline reporting.
  \item Dev-optimized threshold maximizing Macro-F1 under class-balance constraints.
  \item Two-threshold policy for abstention: auto-accept below \(\tau_{low}\), auto-reject above \(\tau_{high}\), and human escalation in between.
\end{itemize}

\textbf{Uncertainty-aware reporting.}
To avoid over-interpreting small differences, we will report bootstrap confidence intervals for AUROC and Macro-F1 on the dev set:
\[
\text{CI}_{95\%}(m) = [q_{0.025}(m^*),\; q_{0.975}(m^*)],
\]
where \(m^*\) denotes metric values across bootstrap resamples.

\textbf{Stress-test slices.}
We will compute metrics on targeted slices (OCR-noisy text, sarcasm-heavy samples, and identity-term-heavy samples) to identify whether aggregate gains hide subgroup regressions.

\begin{table}[h]
\centering
\small
\caption{Planned final-phase reporting bundle.}
\begin{tabularx}{\columnwidth}{lX}
\toprule
Artifact & Purpose \\
\midrule
Overall dev metrics table & Primary model ranking and core comparability \\
Threshold-policy table & Operational tradeoff (automation vs escalation) \\
Bootstrap CI table & Statistical stability of reported gains \\
Slice-wise error table & Detect hidden regressions on difficult subsets \\
Reviewer decision log summary & Validate human-in-the-loop practicality \\
\bottomrule
\end{tabularx}
\label{tab:reportbundle}
\end{table}
...
\subsection{Planned Phase 2: Counterfactual Editing + Safety + Realism}
Planned Phase 2 expands the current system with practical moderation safeguards and generation-based counterfactual intervention.
...
\begin{itemize}[leftmargin=*,nosep]
  \item Counterfactual text editing module to minimally rewrite hateful meme text into non-hateful alternatives while preserving core semantics.
  \item Counterfactual image generation module to produce non-hateful image variants from hateful memes via CLIP latent editing and decoding.
...
\textbf{Counterfactual image-generation direction.}
Given mentor feedback, we will prototype an embedding-space intervention pipeline: encode meme image to CLIP representation, identify and suppress hate-associated latent directions (including SAE-style sparse feature editing as a candidate), then decode the edited representation into a non-hateful image candidate. The edited image will be evaluated for (i) label flip away from hateful, (ii) semantic preservation, and (iii) visual quality.
...
Text counterfactuals & P2-CF-TEXT-1 & Minimal hateful-to-benign text rewrite generator & Label flip rate and semantic preservation \\
Image counterfactuals & P2-CF-IMG-1 & CLIP embedding intervention + decoder generation & Non-hateful conversion rate and visual fidelity \\
Image counterfactuals & P2-CF-IMG-2 & SAE-style sparse latent editing before decode & Edit controllability and artifact rate \\
...
Counterfactual image artifacts & Add visual-quality filtering and human safety review before accepting generated edits. \\
...
Counterfactual intervention quality & Text and image counterfactual pipelines show controlled hateful-to-benign edits & Counterfactual examples, metrics, and failure cases \\
...
\midrule
Modeling track & LoRA/PEFT experiments, training stability checks, ablation updates, and text counterfactual generation. \\
Data/OCR track & OCR pipeline integration, normalization, noisy text stress testing, and image counterfactual decoding pipeline. \\
...
\midrule
Mar 9--Mar 16 & Finalize failure taxonomy; freeze Phase 2 safety policy draft; define counterfactual text/image metrics. \\
Mar 17--Mar 24 & Implement OCR pipeline, confidence-based escalation logic, and text counterfactual baseline. \\
Mar 25--Apr 2 & Prototype image counterfactual pipeline (CLIP latent editing + decoder, including SAE-style variant). \\
Apr 3--Apr 10 & Run LoRA/PEFT experiments and integrated evaluation of counterfactual, OCR, and safety modules. \\
Apr 11--Apr 18 & Final safety review of generated counterfactuals, consolidate results, finalize report/figures, prepare viva notes. \\
\bottomrule
\end{tabularx}
\label{tab:timeline}
\end{table}
